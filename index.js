...
const guild = ("920755627846926346");
...
client.once('ready', async () => {
    ...
    await client.guilds.cache.get(guild)?.commands.set(data);

    // Set Slash Commands Permissions
    setTimeout(async () => {
        console.log('Updating Slash Commands permissions');
        const commandData = await client.guilds.cache.get(guild)?.commands.fetch();
        const permissions = []
        ...
        setTimeout(async () => {
            await client.guilds.cache.get(guild)?.commands.permissions.set({ fullPermissions: permissions });
